...
from sklearn.model_selection import learning_curve, validation_curve, ShuffleSplit, GridSearchCV
...
# view an image (e.g. 32) and print its corresponding label
img_index = 2987
plt.imshow(X[:, :, :, img_index])
plt.show()
print(y[img_index])

# vectorized images (Most Scikit-Learn estimators require that data be strictly 2-dimensional)
...
k_neighbors_classifier = KNeighborsClassifier(n_neighbors=5, weights="uniform", algorithm="auto", leaf_size=30, p=2,
...
def get_learning_curves(classifier):
    title = "{}_Learning_Curves_for_Digit_recognition".format(classifier.__class__.__name__)
    # Cross validation with 100 iterations to get smoother mean test and train
    # score curves, each time with 20% data randomly selected as a validation set.
    cross_validation = ShuffleSplit(n_splits=100, test_size=0.2, random_state=26)
    plot_learning_curve(classifier, title, X_train, y_train, ylim=(0.0, 1.1), cv=cross_validation, n_jobs=4, train_sizes=np.linspace(0.02, 1.0, 50))
...
def get_validation_curve(classifier, param_name, param_range):
    ...
    test_scores_mean = np.mean(test_scores, axis=1)
    test_scores_std = np.std(test_scores, axis=1)
    title = "{}_{}_Validation_Curve_for_Digit_recognition".format(classifier.__class__.__name__, param_name)
    plt.figure()
    plt.title(title)
...
get_validation_curve(decision_tree_classifier, "max_depth", np.linspace(20, 40, 10, endpoint=True))
...
get_validation_curve(neural_network_classifier, "alpha", np.linspace(0.0001, 1000000, 10, endpoint=True))
get_learning_curves(boosting_classifier)
get_validation_curve(boosting_classifier, "n_estimators", np.linspace(1, 200, 10, endpoint=True, dtype=np.int64))
get_validation_curve(boosting_classifier, "learning_rate", np.linspace(0.0000001, 1000000, 10, endpoint=True))
...
get_validation_curve(svm_classifier, "degree", np.linspace(1, 1000000, 10, endpoint=True))
get_learning_curves(k_neighbors_classifier)
get_validation_curve(k_neighbors_classifier, "n_neighbors", np.linspace(1, 3000, 10, endpoint=True, dtype=np.int64))
...
decision_tree_accuracy = grid_search_and_train_model(decision_tree_classifier, {"max_depth": np.linspace(20, 40, 10, endpoint=True), "min_samples_split": np.linspace(0.1, 1.0, 10, endpoint=True)})
neural_network_accuracy = grid_search_and_train_model(neural_network_classifier, { "activation": ["identity", "logistic", "tanh", "relu"], "hidden_layer_sizes": [(100,), (50,), (200,), (50, 50), (100, 100), (50, 150, 50), (50, 50, 50, 50), (150, 50, 50), (150, 50)], "alpha": np.linspace(0.0001, 1000000, 10, endpoint=True)})
boosting_accuracy = grid_search_and_train_model(boosting_classifier, {"n_estimators": np.linspace(1, 200, 10, endpoint=True, dtype=np.int64), "learning_rate": np.linspace(0.0000001, 1000000, 10, endpoint=True), "base_estimator__min_samples_split": np.linspace(1, 32, 32, endpoint=True, dtype=np.int64), "base_estimator__max_depth": np.linspace(0.1, 1.0, 10, endpoint=True)})
svm_accuracy = grid_search_and_train_model(svm_classifier, {'degree': np.linspace(1, 1000000, 10, endpoint=True), 'C': np.linspace(0.001, 10000, 10, endpoint=True)})
k_neighbors_accuracy = grid_search_and_train_model(k_neighbors_classifier, {"n_neighbors": np.linspace(1, 3000, 10, endpoint=True, dtype=np.int64), "p": np.asarray([1, 2], dtype=np.int64)})
